...
export default function MatchDetail({ matchId, className = "" }) {
  ...
  const mapLowercaseScorecard = (node) => {
    ...
    return arr.map((inn) => {
      const title =
        inn.batteamname || inn.batTeamName || `Innings ${inn.inningsid ?? ""}`;
      const score =
        inn.score != null
  ...
  const fetchAll = useCallback(async () => {
    ...
    const mid = String(matchId).match(/\d{1,}/)
      ? Number(String(matchId).match(/\d{1,}/)[0])
      : matchId;
    ...
    try {
      ...
      const centerPayload =
        centerRes.status === "fulfilled" ? tryExtract(centerRes.value) : null;
      const scardPayload =
        scardRes.status === "fulfilled" ? tryExtract(scardRes.value) : null;
      const hscardPayload =
        hscardRes && hscardRes.status === "fulfilled"
      ...
      if ((!innings || innings.length === 0) && scardPayload) {
        if (scardPayload.data && Array.isArray(scardPayload.data.innings))
          innings = scardPayload.data.innings;
      }
  ...
  const players = (() => {
    ...
    const squads =
      ...
      (center.squads ||
        center.teamSquads ||
        (center.data && center.data.squads));
    if (Array.isArray(squads) && squads.length) {
      return squads.flatMap((t) =>
        (t.players || t.squad || []).map((p) => ({
          id: p.id || p.pid,
          name: p.name || p.fullName || p.player,
        }))
      );
    }
    ...
    if (hscard && hscard.matchHeader) {
      ...
      const list = [];
      if (header.team1 && Array.isArray(header.team1.playerDetails))
        list.push(
          ...header.team1.playerDetails.map((p) => ({ id: p.id, name: p.name }))
        );
      if (header.team2 && Array.isArray(header.team2.playerDetails))
        list.push(
          ...header.team2.playerDetails.map((p) => ({ id: p.id, name: p.name }))
        );
  ...
  const matchTitle =
    (center && (center.match?.title || center.series || center.title)) ||
    (hscard &&
      (hscard.matchHeader?.matchDescription ||
        hscard.matchHeader?.matchDesc)) ||
    `Match ${matchId}`;
